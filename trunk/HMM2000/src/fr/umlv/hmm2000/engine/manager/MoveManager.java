...
public class MoveManager {
	...
	private void createCurrentMoveEvent(Location location) {
		Location start = Engine.currentEngine().selectionManager()
				.getSelectedLocation();
		if (start != null) {
			MapForegroundElement element = Engine.currentEngine()
	...
	public void performMoveEvent(MoveEvent event) {
		for (Step move : event.getMoves()) {
			if (move.isRecheable()) {
				...
				MapForegroundElement element = this.map.getMapForegroundElementAtLocation(l);
				if (element != null) {
					EncounterEvent ecounterEvent = new EncounterEvent(
							this.currentSource, element, move.getStart(), l);
					if (!Engine.currentEngine().encounterManager().perform(
							ecounterEvent)) {
						break;
					}
				}
				...
				Engine.currentEngine().selectionManager()
						.perform(move.getEnd());
			}
		}
	}
